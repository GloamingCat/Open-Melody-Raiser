...
class SongCreationDialog(tkinter.simpledialog.Dialog):
	...
	def body(self, frame):
		# Structure/rhytm params
		self.params["bars"] = tkinter.StringVar(value="1 1 2")
		self.params["bpb"] = tkinter.StringVar(value=4)
		self.params["bpm"] = tkinter.StringVar(value=60)
		...
		label.grid(column=0, row=0, sticky=tkinter.W, padx=5, pady=5)
		box = ttk.Spinbox(structFrame, from_=2, to=16, width=4, textvariable = self.params["bpm"])
		box.grid(column=1, row=0, sticky=tkinter.EW, padx=5, pady=5)
		# Beats per bar
		label = tkinter.Label(structFrame, text="Beats per Bar")
		label.grid(column=2, row=0, sticky=tkinter.W, padx=5, pady=5)
		box = ttk.Spinbox(structFrame, from_=2, to=16, width=4, textvariable = self.params["bpb"])
		...
		label.grid(column=0, row=1, sticky=tkinter.W, padx=5, pady=5)
		box = tkinter.Entry(structFrame, textvariable = self.params["bars"])
		box.grid(column=1, row=1, columnspan=2, sticky=tkinter.EW, padx=5, pady=5)
		structFrame.pack(side=tkinter.TOP, expand=True)
		# Melody
		melodyFrame = BarCreationFrame(frame, self.params, "mel")
		melodyFrame.pack(side=tkinter.LEFT, expand=True)
		# Harmony
		harmonyFrame = BarCreationFrame(frame, self.params, "har")
